...
def gen_scaled_libs(ref_libs:str, lib_directory:str,
    ...
    # Delay factors are the first elements of the scaling factors
    delay_factors = " ".join([str(x) for x in scaling_factors[:1]])
    cmd = f"./delay_scaling.sh {delay_factors}"
    os.system(f"{cmd}")
    
    # Power factos are the next 2 elements of the scaling factors
    # separated by space
    power_factors = " ".join([str(x) for x in scaling_factors[1:3]])
...
if __name__ == '__main__':
    ...
    ref_lib = f"{PROJ_DIR}/scripts/library_scaling/scaled_lib_ref_ng45"
    # example input scaling factor for ng45
    # -0.2 -0.2 -0.2
