...
import numpy as np
import re
from extract_scaled_loss_ng45 import extract_scale_loss
...
class raytune:
    def autotuneObjective(self, config):
        scaling_factor = [config.get('delayScale'),
                          config.get('intPowerScale'),
                          config.get('switchPowerScale')]
    ...
    def __init__(self, num_samples, num_jobs):
        ...
        self.ref_csv = f"{PROJ_DIR}/benchmarks/jpeg_encoder/golden_sample_a7.csv"
        self.num_samples = num_samples
        self.num_jobs = num_jobs # number of cpus used by tune
        self.config = {
            # ------- general parameters ------------*
            "delayScale":           tune.uniform(-0.4, -0.1),
            "intPowerScale":        tune.uniform(-0.45, -0.05),
            "switchPowerScale":     tune.uniform(-0.45, -0.05),
            }
